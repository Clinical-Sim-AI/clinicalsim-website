...
export const releases: Release[] = [
  {
    ...
    userFacing: [
      "**Surveys handle rating grids, follow-ups, write-ins and section headings.** A grid puts one statement per row and one scale across the top, so a thirteen-statement agreement block reads as a block instead of thirteen separate questions. The same block can run before and after a case, which is what makes a change score possible. A follow-up question stays hidden until the answer that calls for it: pick your role and you get the question about years since fellowship, answer \"yes\" to prior training and you get asked which programs. A choice can offer \"Other\" with a box to specify. Headings and short instructions can sit between questions, so a long instrument breaks into sections instead of running on. We built this for a peds hem-onc study comparing AI-written feedback with coach-led debriefing, and it runs on the pre and post surveys every case already has.",
      "**Grids read on a phone.** On a narrow screen each statement becomes its own card with five full-width answer buttons. No pinching, no sideways scrolling.",
      "**Answers past the first page of a survey are no longer thrown away.** A survey can run to more than one page, and learners were answering every page of it. But the two places that read the results back, the survey panel on a conversation and the roster download, only ever looked at page one. Everything after it was collected and then dropped. Both now read the whole survey.",
      "**Copying a case brings its surveys with it.** Duplicating a simulation used to leave both surveys behind without saying so, and the copy looked identical to its source in every other respect. It copies them now.",
      "**Each grid statement is still its own column in the roster download.** One statement, one column, the same as any single-answer question, so the export opens in a spreadsheet and the before and after columns sit next to each other.",
    ],
    team: [
      "Survey visibility is worked out once, in one place, and the renderer and the submitter both read that answer. They used to decide separately, and on a chain where question A controls B and B controls C they could disagree: the learner saw C, filled it in, and the submitter dropped the answer with no error and nothing in the logs. That can't happen now, and a condition pointing at a later question is simply correct rather than quietly lossy.",
      "A condition an author gets wrong now fails open. Dangling, circular, self-referential, or pointing at a choice that doesn't exist, and the question shows anyway. Hiding a required question over a typo would strand the learner in front of a form that can't be submitted. Those problems are reported to the author and block publishing, but they never reach the learner as a \"couldn't be displayed\" notice.",
      "Submitting an incomplete survey now scrolls to the field that needs an answer, including a grid row or a write-in box. Both were invisible to the old lookup, so focus stayed on the submit button and nothing told the learner what was wrong.",
      "The survey builder holds on to what it doesn't recognize. It rebuilds the whole survey from what's on screen on every keystroke, so anything it didn't model got deleted the next time someone opened that survey and changed something unrelated. A rating question written by script used to come back as a radio group with invented choices. Opening and saving a survey without touching it is now asserted byte for byte identical, which also stops every untouched survey reporting \"the survey changed\" on open.",
      "Grid rows get their identifier once, when they're created. Paste thirteen statements and you get thirteen rows, and inserting one at the top later leaves the existing rows their identity, so answers already collected stay attached to the statement they were about. Column presets for the five-point agreement scale and the AI-versus-coach scale make a thirteen-item grid one paste instead of sixty-five cells, and stop a before and after pair drifting apart through retyped labels.",
      "None of this needed a database change, and surveys still sit outside the fingerprint that spots duplicate case versions, so publishing behaves exactly as it did.",
      "A new development machine comes up in 1m47s, down from 25m38s on the worst measured run. That baseline stacked three unlucky conditions at once. A typical second workspace was about ten minutes, so the honest steady-state figure is closer to five and a half times faster. Caches are now shared across workspaces instead of rebuilt per branch, so the browser download and the database images are already there. The dependency fetch moved, so a lockfile change no longer re-runs a four-minute install and leaves another multi-gigabyte image behind. And the three large downloads that repeated per workspace are either baked into the image or resumable, so a stalled download picks up where it stopped instead of starting over.",
      "Staging builds are debuggable in the browser now, with real component names in React DevTools and browser source maps, and production's posture is untouched. That took two separate mechanisms, because component names come from the running code and never from source maps: shipping maps alone would have left the component tree reading single letters. Both switches are allowlists on the deploy target rather than \"anything that isn't production\". If that variable were renamed or dropped, the cost is staging losing its debuggability, not the full source of the production app becoming downloadable. Production leaves the source-map option unset rather than off, because off is a third state that stops maps being generated at all and would silently un-symbolicate every production stack trace. Both deploys check the result from outside afterwards: staging has to sit behind its auth wall, production has to serve no maps.",
      "Still to come, and data rather than code: the two study instruments themselves, as staging drafts. Which case version each attaches to is a study-operations call, so we'll confirm it with the coordinator. Two things to raise with her at the same time. The post-survey's second section skips two numbers, and it still carries the pre-survey's \"before beginning this study, which form do you expect\" question, which reads as a copy-paste leftover.",
    ],
  },
  {
    id: "2026-09-08",
  ...
  },
  {
    ...
    userFacing: [
      "**Build a project from a ready-made curriculum.** New Project used to open in custom mode, leaving you to assemble a program case by case out of a long two-column grid. It now opens on the templates we've built for you: a searchable list grouped by specialty and sorted by level, with the cases laid out for review before you fill in any project details. Expand any case to read its role, scenario, and objectives without leaving the page, so you can see what a template actually teaches before you adopt it. The in-app Catalog and the Simulations page carry the same Simulations / Project templates switch, and picking a template there carries it straight into New Project. Adopting one creates the project, pins its cases in order, and grants your organization access in a single step. Learners see no change to their own catalog.",
      "**Put a project's cases in the order you teach them.** Program directors and admins can reorder the simulations in a project with Up and Down controls that work with a keyboard and a screen reader. Learners see stable case numbers as they work through the list, so Case 4 stays Case 4 once the ones before it are finished and hidden, and they can still start any case at any time.",
      "**You can see when the patient is thinking.** There was a stretch after you finished speaking where nothing on screen changed, and no way to tell whether the patient was composing a reply or the call had quietly dropped. The orb now tells you a reply is on its way, including through the silence that follows a spoken \"um\" or \"well\". If the wait runs past ten seconds you get a message saying so, and the call stays connected throughout. The indicator respects reduced-motion settings and announces itself once to a screen reader rather than chattering.",
      "**The live call actually looks like one.** The moment a simulation connected, the orb lost almost all of its visual presence. The ring around it vanished, and the halo meant to pulse with the voice was running at roughly four percent of full strength, below the point where an eye can see it. On a pale page that left a pale disc barely moving for the whole session. The ring is now drawn in a color that reads against the background instead of disappearing into it, and the halo responds across the range real speech occupies, so you can see at a glance who is talking and how loudly.",
      "**Feedback on conversation frameworks reads like coaching, not a checklist.** Frameworks like NURSE are a set of moves to reach for, not boxes to tick in order, and scoring them as a percentage sent exactly the wrong message. Those rubrics now come back as a plain Yes or No per element with the coaching attached, and no score, no scorecard table, and no total. Three bugs went with the redesign: every check row read \"Not answered\" while the grade itself held six confident answers; each element split into two rows, one for the finding and one for the quote, and inconsistently at that; and the quote that earned a Yes now sits inside the finding, so you read what you said and why it worked together, without a click.",
      "**If the voice service is down, you find out before you start.** Voice failures used to stay invisible until you tried to run a case and it silently refused. The status the app checks is now the same request it makes to start a real simulation, so an outage can't hide behind a healthy-looking check. When one is confirmed, you see a notice and the Start button holds rather than dropping you into a session that can't begin. Conversations already in progress keep running, and the moment a fresh attempt looks possible the gate lifts on its own. A monitoring problem on our side will never leave you stranded behind stale bad news.",
    ],
    team: [
      "Our health checks now test the operations the product actually performs instead of the vendor status call sitting next to them. The old voice check answered a cheerful 200 in under 200ms throughout a half-hour outage in which no learner could start a simulation. Each check now exercises the real thing: an actual inference call, the specific storage bucket and object path, the grading queue, database write readiness. Each keeps its own timing and a named diagnosis, with \"we never configured this\" reported separately from \"we asked and it failed\", and they run against independent deadlines so one stalled dependency can't hide the state of the others.",
      "A watchdog runs every five minutes and rechecks a failing critical dependency twenty seconds later before it says anything, so a blip doesn't page anyone. A confirmed failure goes to our error monitoring and a dedicated Slack channel. It paints nothing in the product on purpose: \"the mail provider's domain is unverified\" is not something a learner can act on. Optional credentials stay optional, and anything we never configured doesn't page.",
      "The email suppression list is reconciled against the mail provider's instead of guessed at. Its only feed was an inbound notification, and no job anywhere could pull the two lists back together, so once they drifted apart they stayed apart. Measuring against live data before changing anything showed the drift running the opposite way to what we expected: nothing was missing from our copy, but four addresses we were still refusing to mail had already been cleared on the provider's side. Un-suppressing an address from the admin tools now genuinely un-suppresses it.",
      "National ACGME benchmarks are in the database and wired through, switched off. Milestone comparisons had been using generic year-in-training targets and could match identical competency codes across different specialties. This adds 7,748 national reference points from the 2025 report, a reviewed mapping to our own rubrics, optional program and year fields on a profile, and specialty-qualified progress, heatmaps, and drill-down. Organization overrides and per-case ceilings still take precedence, and learners never see peer rankings or distributions. Turning it on in production is a separate decision.",
      "A database update that had been blocking production deploys is fixed. The national benchmark data shipped as nearly 10 MB of JSON inside a single statement, which the deployment tool's parser hung on indefinitely. The database itself applied it in seconds. It's now regenerated as bounded batches, staged and validated before anything is written, with the original checksum and all 7,748 references preserved. Verified against both the production and staging versions of the tool, from a clean database and from a replay.",
      "The grader's Yes/No answers are a contract now rather than a lucky accident. The app had been reading a richer answer-plus-reasoning shape that we never actually asked for, and the model had simply been generous. The next model revision could have stopped sending it and quietly dropped the per-item coaching on every unscored rubric, with no test failing anywhere. We now ask for the shape we read.",
      "Filler words the AI speaks while composing a reply (\"um\", \"let me think\") are stripped from what the grader sees, matching only complete tokens at the start of a turn so a learner's own words are never touched. What's stored and displayed in the transcript is unchanged. A read-only checker also validates that each environment points at the voice agent it's supposed to.",
      "Admins can author the curriculum templates behind the new project flow: create and edit drafts, reorder cases, preview, publish immutable revisions, archive, and see adoption and current learner-assignment numbers per template. A published template can grant access to approved cases that aren't in the public catalog without changing their visibility flags, and later revisions or archiving leave projects already built from a template untouched. Templates carry through our staging-to-production content promotion.",
    ],
  },
  {
    id: "2026-08-24",
